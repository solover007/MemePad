...
describe('presale_hub', () => {
    ...
    const presalePrice = new anchor.BN(50_000 * DENOMINATER); // 50000/1sol
    const listingPrice = new anchor.BN(40_000 * DENOMINATER); // 40000/sol
    ...
    it('should create a presale', async () => {
        ...
        expect(presaleAccount.presalePrice.toNumber()).to.equal(presalePrice.toNumber());
        ...
        // expect(presaleAccount.saleType).to.equal(saleType);
    ...
    it('should buy tokens with SOL', async () => {
        ...
        console.log('user sol balance: ', await connection.getBalance(user.publicKey)); const tokenAmount = new anchor.BN(50_000);
        const solAmount = new anchor.BN(1000_000_000); // 1 sol
        ...
        // simulateTimeTravel()
        try {
            let tx = await program.methods.buyTokenWithSol(tokenAmount, solAmount)
                .accounts({
            ...
            console.log("userStatePDA QuoteToken Amount:", Number(userStateInfo.buyQuoteTokenAmount.toString()));
            ...
            console.log("User myBaseToken Balance: ", await getWalletTokenBalance(user, myBaseToken, 9));
    ...
    it('should perform emergency withdrawal', async () => {
        ...
        try {
            let tx = await program.methods.emergencyWithdraw()
                .accounts({
                    buyer: user.publicKey,
                    presaleState: presalePDA,
                    userState: userStatePDA,
