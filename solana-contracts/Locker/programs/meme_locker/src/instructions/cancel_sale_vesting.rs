...
#[derive(Accounts)]
pub struct CancelSaleVesting<'info> {
    #[account(mut)]
...
pub fn handle(
    ...
    if seller.sale_vests[sale_vesting_id as usize].max_bid_price > 0 {
        ...
        invoke_signed(
            &system_instruction::transfer(
                &accts.vault.key(),
                &accts.user.key(),
                seller.sale_vests[sale_vesting_id as usize].amount * seller.sale_vests[sale_vesting_id as usize].max_bid_price,
            ),
            &[
    ...
    // back vesting to seller
    let index = seller.sale_vests[sale_vesting_id as usize].original_id as usize;
    seller.vests[index].current_amount += seller.sale_vests[sale_vesting_id as usize].amount;
    ...
    msg!("Successfully cancel sale vesting");
