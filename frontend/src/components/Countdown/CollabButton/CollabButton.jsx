...
const CollabButton = () => {
    return(
        <div
        className=" relative pl-6 py-1 lg:w-[15rem] md:flex justify-end flex-row items-center text-left text-black z-[100]"
        
      >
        <CopyButtonBg className="md:absolute relative block h-full  lg:left-4 top-0 z-[99]" />
        <p className="absolute md:static top-[6%] -right-[15%] z-[100] font-extrabold mr-[32%] lg:mr-[18%] py-4 text-xs lg:text-lg uppercase">
          whitepaper
        </p>
      </div>
    )}
    ...
    const CopyButtonBg = ({className})=>(
        ...
        className={`${className} w-36 h-full lg:w-auto`}
      >
        <mask
